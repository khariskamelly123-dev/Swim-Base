...
@section('title', 'Login Klub - Swim Base')
...
@section('content')
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .login-box {
            background-color: rgba(0, 0, 0, 0.55); padding: 2rem;
            border-radius: 16px; margin-right: 150px; margin-top: 50px;
            width: 400px; box-shadow: 0 0 15px rgba(0, 0, 0, 0.5);
        }
        .login-box label { display: block; font-weight: 600; margin-bottom: 0.25rem; color: #eee; }
        .input-icon { position: relative; width: 100%; }
        .input-icon .icon {
            width: 22px; height: 22px; position: absolute; left: 14px; top: 23px;
            transform: translateY(-50%); opacity: 0.9;
        }
        .input-icon input {
            width: 100%; height: 45px; padding-left: 48px; border-radius: 8px;
            border: 2px solid rgba(255, 255, 255, 0.5); background: transparent;
            color: #fff; font-size: 15px; outline: none; margin-bottom: 1rem;
        }
        ...
        .login-btn {
            ...
            font-size: 1.1rem; cursor: pointer; margin-bottom: 1rem; transition: 0.3s ease;
        }
        ...
        .signup-text {
            text-align: center; font-size: 0.9rem; color: #ccc; margin-top: 1rem;
        }
        ...
        @media (max-width: 900px) {
            ...
            .login-box { margin: 0; width: 90%; max-width: 360px; }
        }
    </style>
    ...
    <div class="container">
        <div class="welcome-text">
            ...
            <p>Silakan login untuk mengelola data klub Anda</p>
        </div>
        ...
        <div class="login-box">
            ...
            {{-- UPDATE: Action mengarah ke route 'club.login.process' --}}
            <form action="{{ route('club.login.process') }}" method="POST">
                @csrf

                <label for="email">Email Resmi Klub</label>
                <div class="input-icon">
                    <img src="{{ asset('images/email-icon.png') }}" class="icon">
                    
                    {{-- UPDATE: name="email" (bukan email_resmi) --}}
                    <input type="email" id="email" name="email" 
                           placeholder="contoh: klub@gmail.com" 
                           value="{{ old('email') }}" required autofocus>
                </div>
                
                {{-- Error Message Email --}}
                @error('email')
                    <div style="color: #ff6b6b; font-size: 0.85rem; margin-top: -10px; margin-bottom: 10px;">
                        {{ $message }}
                    </div>
                @enderror
